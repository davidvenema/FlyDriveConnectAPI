...
def list_cars(db: Session = Depends(get_db), airport_id: int | None = None, status: str | None = None):
    q = db.query(Car)
...
@router.put("/{cars_id}", response_model=CarOut)
def update_car(cars_id: int, payload: CarUpdate, db: Session = Depends(get_db)):
...
@router.delete("/{cars_id}")
def delete_car(cars_id: int, db: Session = Depends(get_db)):
    obj = db.query(Car).get(cars_id)
    if not obj:
        raise HTTPException(404, "Car not found")
    db.delete(obj)
